    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div class="mb-4">
        <div class="relative max-w-xs">
          ...
          <input v-model="search" type="text" placeholder="Search contracts…" class="block w-full rounded-lg border border-slate-200 py-2 pl-9 pr-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
        </div>
      </div>
          ...
          <tbody class="divide-y divide-slate-100 bg-white">
            ...
            <tr v-if="filtered.length === 0">
              <td colspan="7" class="px-6 py-16 text-center">
                <p class="text-sm text-slate-500">{{ search ? 'No contracts match your search.' : 'No contracts found.' }}</p>
              </td>
            </tr>
          </tbody>
        </table>
